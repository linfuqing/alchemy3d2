	...
	CEffectCode::CInstructionSetInterface& ShaderDefiner)
{
	...
	ShaderDefiner.AppendInstruction(CCompilerInterface::SUB, uReflection, uOne, puAttributes[ATTRIBUTE_FACING]);

	ShaderDefiner.AppendInstruction(CCompilerInterface::POW, uReflection, uReflection, puConstant[CONSTANT_FRESNEL_POW]);

	ShaderDefiner.AppendInstruction(CCompilerInterface::MUL, uReflection, puConstant[CONSTANT_INVERSE_BIAS], uReflection);

	ShaderDefiner.AppendInstruction(CCompilerInterface::ADD, uReflection, puConstant[CONSTANT_FRESNEL_BIAS], uReflection);

	ShaderDefiner.AppendInstruction(CCompilerInterface::SAT, uReflection, uReflection, 0);

	ShaderDefiner.AppendInstruction(CCompilerInterface::MUL, uSourceColor, uSourceColor, uReflection);

	ShaderDefiner.AppendInstruction(CCompilerInterface::SUB, uReflection, uOne, uReflection);

	ShaderDefiner.AppendInstruction(CCompilerInterface::MUL, uDestinationColor, uDestinationColor, uReflection);

	ShaderDefiner.AppendInstruction(CCompilerInterface::ADD, uOutputColor, uSourceColor, uDestinationColor);
