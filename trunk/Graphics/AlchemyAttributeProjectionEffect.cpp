...
const CEffectCode::GLOBALVARIABLE CAttributeProjectionEffect::sm_CONSTANS[CAttributeProjectionEffect::TOTAL_CONSTANTS] = { {CEffectInterface::VERTEX_SHADER, CEffectCode::MATRIX_VARIABLE, CRenderMethod::WORLD_VIEW_PROJECTION_MATRIX, 1, 0} };
...
const CEffectInfoInterface::SAMPLER           * CAttributeProjectionEffect::GetSamplers           (UINT& uSamplerCount          )
{
...
const CEffectCode::ELEMENTPARAMETER            * CAttributeProjectionEffect::GetAttributes        (UINT& uAttributeCount        )
{
...
const CEffectCode::ELEMENTPARAMETER            * CAttributeProjectionEffect::GetVarying           (UINT& uVaryingCount          )
{
	...
	CEffectCode::CPassDefiner::CEffectCodeUtils& EffectCodeUtils)
{
	...
	if(pOutput->uHandle && !pOutput->bIsUsed)
	{
		if(m_AttributeVariable[ATTRIBUTE_VARIABLE].ElementType == CVertexDeclaration::FLOAT4)
			VertexShaderDefiner.AppendInstruction(CCompilerInterface::M44, pOutput->uHandle, pAttributeVariables[ATTRIBUTE_VARIABLE].uSource, puConstants[CONSTANT_WORLD_VIEW_PROJECTION_MATRIX]);
		else
		{
			VertexShaderDefiner.AppendInstruction(CCompilerInterface::M34, pOutput->uHandle, pAttributeVariables[ATTRIBUTE_VARIABLE].uSource, puConstants[CONSTANT_WORLD_VIEW_PROJECTION_MATRIX]);

			UINT w = 3;
			VertexShaderDefiner.AppendInstruction(CCompilerInterface::MOV, pOutput->uHandle, &w, 1, pAttributeVariables[ATTRIBUTE_VARIABLE].uSource, &w, 1, 0, ALCHEMY_NULL, 0);
		}
	}
