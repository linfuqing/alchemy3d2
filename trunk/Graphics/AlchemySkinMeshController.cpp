...
bool CSkinMeshController::Create(CMesh& Mesh, CSkinMeshController::SKINTYPE SkinType, CSkinInfo& SkinInfo, CBone& Root, const CEffectCode::ELEMENTPARAMETER* pSkinElementParameters, UINT uElementCount, UINT uMaxBoneInfluence, CEffectCode* pEffectCode)
{
	m_pSourceMesh = &Mesh;
	...
	if(SkinType == SKIN_HARDWARE)
	{
		...
		if(pEffectCode)
			uMatrixIndexOffset = pEffectCode->GetRegisterCount(CEffectCode::MATRIX4X3_VARIABLE) >> pEffectCode->GetCompiler().GetByteAlignmentShift();
		...
		if(pMesh)
		{
			...
			if(pEffectCode)
			{
				...
				m_pHardwareSkinEffect->Create(uMaxBoneInfluence ? Mininum(uBoneCount, uMaxBoneInfluence) : uBoneCount, uMaxVertexInfluence, pSkinElementParameters, uElementCount);
			}
		}
	}
...
bool CSkinMeshController::Create(CMesh& Mesh, CSkinMeshController::SKINTYPE SkinType, CSkinInfo& SkinInfo, CBone& Root, UINT uMaxBoneInfluence, CEffectCode* pEffectCode)
{
	...
	return Create(Mesh,SkinType, SkinInfo, Root, SkinElementParameters, uSkinElementParameterCount, uMaxBoneInfluence, pEffectCode);
}
...
{
	m_pRoot = ALCHEMY_NULL;
	ALCHEMY_DEBUG_DELETE(m_pSkinInfo);
	ALCHEMY_DEBUG_DELETE_ARRAY(m_ppBones);
