...
const CEffectCode::GLOBALVARIABLE CAttributeWorldSpaceEffect::sm_CONSTANS[CAttributeWorldSpaceEffect::TOTAL_CONSTANTS] = { {CEffectInterface::VERTEX_SHADER, CEffectCode::MATRIX_VARIABLE, CRenderMethod::WORLD_MATRIX, 1, 0} };
...
const CEffectInfoInterface::SAMPLER           * CAttributeWorldSpaceEffect::GetSamplers           (UINT& uSamplerCount          )
{
...
const CEffectCode::ELEMENTPARAMETER            * CAttributeWorldSpaceEffect::GetAttributes        (UINT& uAttributeCount        )
{
...
const CEffectCode::ELEMENTPARAMETER            * CAttributeWorldSpaceEffect::GetVarying           (UINT& uVaryingCount          )
{
	...
	CEffectCode::CPassDefiner::CEffectCodeUtils& EffectCodeUtils)
{
	...
	if(pOutput->uHandle && !pOutput->bIsUsed)
	{
		if(m_AttributeVariable[ATTRIBUTE_VARIABLE].ElementType == CVertexDeclaration::FLOAT4)
			VertexShaderDefiner.AppendInstruction(CCompilerInterface::M44, pOutput->uHandle, pAttributeVariables[ATTRIBUTE_VARIABLE].uSource, puConstants[CONSTANT_WORLD_MATRIX]);
		else
		{
			VertexShaderDefiner.AppendInstruction(CCompilerInterface::M33, pOutput->uHandle, pAttributeVariables[ATTRIBUTE_VARIABLE].uSource, puConstants[CONSTANT_WORLD_MATRIX]);
