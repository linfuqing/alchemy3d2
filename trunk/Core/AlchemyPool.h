...
namespace alchemy
{
	...
	protected:
		typedef enum
		{
			...
			ARRAY_MEMBER_COUNT_SHIFT = ALCHEMY_SHIFT(ARRAY_MEMBER_COUNT)
		}CONTAINS;
	...
	public:
		...
		ALCHEMY_INLINE bool IsEnd(const CConstantIterator& Iterator)
		{
		...
		ALCHEMY_INLINE bool Set(const TYPE& Value, UINT uIndex)
		{
			...
			{
				ALCHEMY_SET_BIT( pArray->uFlag, uIndex - (uArrayIndex << ARRAY_MEMBER_COUNT_SHIFT) );
		...
		CPool<TYPE>& operator+=(const CPool<TYPE>& Pool)
		{
			...
			Iterator = Pool.GetBeginIterator();
		...
		bool GetIndexOf(UINT& uOutput, TYPE Object)
		{
			EQUALFUNCTION EqualFunction;

			CConstantIterator Iterator = GetBeginIterator();
	...
	private:
		...
		bool __GetBegin(CConstantIterator& Iterator)const
		{
			Iterator.m_pArray     = m_pPoolList;
			Iterator.m_uIndex     = 0;
			Iterator.m_uBaseIndex = 0;

			if(Iterator.m_pArray != ALCHEMY_NULL)
			{
				if( !ALCHEMY_TEST_BIT(m_pPoolList->uFlag, 0) )
					++ Iterator;
