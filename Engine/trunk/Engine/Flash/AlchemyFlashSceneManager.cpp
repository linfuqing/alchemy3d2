...
CFlashSceneManager::CFlashSceneManager(CFlashContext3D& Context3D) :
CSceneManager(Context3D, &ALCHEMY_FLASH_SCENE_NODE_FACTORY)
{
}
...
{
	return AS3_Array("AS3ValType, PtrType, PtrType", static_cast<const AS3TYPE>( static_cast<const CFlashCamera&>( GetSceneCamera() ) ), static_cast<const FLOAT*>(m_AmbientLight), &m_BackgroundColor);
}
...
CGraphicsResource* CFlashSceneManager::_Create(CGraphicsResource::TYPE Type)
{
	...
	case CGraphicsResource::VERTEX:
        ...
        pResource = CSceneManager::_Create(Type);
		break;
