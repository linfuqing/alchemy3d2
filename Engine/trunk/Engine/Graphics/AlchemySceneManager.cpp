...
//#include <as3.h>
CSceneNode* CSceneManager::CreateScene(
									   CMeshManager::HANDLE hSceneDataHandle, 
									   CSkinMeshController::SKINTYPE SkinType, 
									   CVector<CSceneObject*>* pSceneObjects, 
									   CVector<CBone*>* pRootBone, 
									   CVector<RESOURCEEINFO>* pMaterialInfo, 
									   CVector<RESOURCEEINFO>* pTextureInfo, 
									   CVector<RESOURCEEINFO>* pMeshInfo, 
									   ISceneNodeFactory* pSceneNodeFactory)
{
	...
	ISceneNodeFactory& SceneNodeFactory = pSceneNodeFactory ? *pSceneNodeFactory : m_SceneNodeFactory;

	CSceneNodeUpdater* pRootScene = &SceneNodeFactory.CreateSceneNodeUpdater();
		...
		if( !ppSceneNode || !(*ppSceneNode) )
		{
			...
			if(pNodeData)
			{
				...
				case CMeshManager::NODE:
					pSceneNode = &SceneNodeFactory.CreateSceneNode();
				...
				case CMeshManager::BONE:
					pBone = &SceneNodeFactory.CreateBone();
				...
				case CMeshManager::OBJECT:
					{
						pSceneObject = &SceneNodeFactory.CreateSceneObject();
	...
	while( CVector<UINT>::IsValuable(Iterator) )
	{
		...
		if(ppSceneNode && ( pSceneObject = dynamic_cast<CSceneObject*>(*ppSceneNode) ) != ALCHEMY_NULL)
		{
			...
			if(ppSceneNode)
			{
				ppBone = BoneMap.Get(*ppSceneNode);
				pBone = ppBone ? *ppBone : ALCHEMY_NULL;
				if(!pBone)
				{
					pBone = dynamic_cast<CBone*>(*ppSceneNode);
					if(pBone)
					{
						BoneMap.Insert(*ppSceneNode, pBone);
