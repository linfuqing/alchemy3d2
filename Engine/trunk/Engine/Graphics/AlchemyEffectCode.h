...
namespace alchemy
{
	...
	public:
		...
		typedef struct 
		{
			UINT uCurrentInstruction;
			UINT uNextIndex;
		}INSTRUCTIONINDEX, * LPINSTRUCTIONINDEX;

		typedef struct  
		{
			UINT uVertexInstructionIndices;
			UINT uFragmentInstructionIndices;
			...
			UINT uIndex;
			UINT uNext;
		}RENDERTARGETINSTRUCTION, * LPRENDERTARGETINSTRUCTION;
		...
		typedef struct  
		{
			...
			UINT uRenderTargetInstructionSet;
			//UINT uVertexUserVariables;
		...
		static UINT SerializeAddress(const UINT* puAddress, UINT uAddressCount);
		static void UnserializeAddress(UINT uAddress, PUINT puOutputAddress = ALCHEMY_NULL, PUINT puOutputAddressCount = ALCHEMY_NULL);
		static void UnserializeAddress(UINT uAddress, const UINT* puSourceAddress, UINT uSourceCount, PUINT puOutputAddress = ALCHEMY_NULL, PUINT puOutputAddressCount = ALCHEMY_NULL);
		...
		const INSTRUCTION                * GetInstruction               (UINT uHandle)const;
		const UINT                       * GetInstructionSet            (UINT uHandle)const;

		const INSTRUCTIONINDEX           * GetInstructionIndex          (UINT uHandle)const;
		const UINT                       * GetInstructionIndices        (UINT uHandle)const;

		const RENDERTARGETINSTRUCTION    * GetRenderTargetInstruction   (UINT uHandle)const;
		const UINT                       * GetRenderTargetInstructionSet(UINT uHandle)const;
		...
		const CSmartPointer<CBuffer>     * GetBuffer                    (UINT uHandle)const;
		...
		UINT AppendCodeInstruction(UINT uInstruction, PUINT puUserVaraible = ALCHEMY_NULL, UINT uDestination = 0, bool bIsUpdateMap = true, PUINT puConstantInstruction = ALCHEMY_NULL);
 
		UINT AndMatchingAddress(
			UINT uSource, 
			UINT uDestination, 
			UINT uTarget, 
			const UINT* puSourceAddress = ALCHEMY_NULL, 
			UINT uSourceAddressCount = 0,
			PUINT puOutputAddress = ALCHEMY_NULL, 
	...
	private: 
		...
		CPool<INSTRUCTIONINDEX>                                                      m_InstructionIndexPool;
		CPool<UINT>                                                                  m_InstructionIndicesPool;

		CPool<RENDERTARGETINSTRUCTION>                                               m_RenderTargetInstructionPool;
		CPool<UINT>                                                                  m_RenderTargetInstructionSetPool;
	...
	ALCHEMY_INLINE const CEffectCode::VARIABLE          * CEffectCode::GetVariable          (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CEffectCode::GLOBALVARIABLE    * CEffectCode::GetGlobalVariable    (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CEffectCode::USERVARIABLE * CEffectCode::GetUserVariable           (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const UINT                           * CEffectCode::GetUserVariables     (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const IEffect::SAMPLERDECLARATION    * CEffectCode::GetSamplerDeclaration(UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CEffectCode::STRUCTDECLARATION * CEffectCode::GetStructDeclaration (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const UINT                           * CEffectCode::GetStruct            (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CEffectCode::INSTRUCTION                * CEffectCode::GetInstruction               (UINT uHandle)const
	{
		if(uHandle)
			return m_InstructionPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const UINT                       * CEffectCode::GetInstructionSet            (UINT uHandle)const
	{
		if(uHandle)
			return m_InstructionSetPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const CEffectCode::INSTRUCTIONINDEX           * CEffectCode::GetInstructionIndex          (UINT uHandle)const
	{
		if(uHandle)
			return m_InstructionIndexPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const UINT                       * CEffectCode::GetInstructionIndices        (UINT uHandle)const
	{
		if(uHandle)
			return m_InstructionIndicesPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const CEffectCode::RENDERTARGETINSTRUCTION  * CEffectCode::GetRenderTargetInstruction  (UINT uHandle)const
	{
		if(uHandle)
			return m_RenderTargetInstructionPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const UINT                           * CEffectCode::GetRenderTargetInstructionSet(UINT uHandle)const
	{
		if(uHandle)
			return m_RenderTargetInstructionSetPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const CEffectCode::FUNCTION          * CEffectCode::GetFunction          (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CEffectCode::SHADERPARAMETER   * CEffectCode::GetShaderParameter   (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const UINT                           * CEffectCode::GetShaderParameters  (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CEffectCode::SHADERFUNCTION    * CEffectCode::GetShaderFunction    (UINT uHandle)const
	{
		if(uHandle)
	...
	ALCHEMY_INLINE const CSmartPointer<CBuffer>     * CEffectCode::GetBuffer                    (UINT uHandle)const
	{
		if(uHandle)
			return m_BufferPool.Get(uHandle - 1);

		return ALCHEMY_NULL;
	}

	ALCHEMY_INLINE const CEffectCode::PASS              * CEffectCode::GetPass              (UINT uHandle)const
	{
		if(uHandle)
