			...
			for(UINT i = 0; i < uGlobalInpuTCount; ++ i)
			{
				if(bIsActivate || !pGlobalInput->bIsStatic)
				{
					...
					case MATRIX4X3_VARIABLE:
						if(bIsPointer)
							Effect.SetShaderMatrixPointerArray(pGlobalInput->ShaderType, pGlobalInput->uIndex, reinterpret_cast<const PFLOAT*>(pGlobalInput->pValue), 4, 3, uRowOffset, pGlobalInput->uCount);
						else
							Effect.SetShaderMatrixArray(pGlobalInput->ShaderType, pGlobalInput->uIndex, reinterpret_cast<const FLOAT*>(pGlobalInput->pValue), 4, 3, uRowOffset, uOffset, pGlobalInput->uCount);
						break;
...
void CEffectCode::UnserializeAddress(UINT uAddress, const UINT* puSourceAddress, UINT uSourceCount, PUINT puOutputAddress, PUINT puOutputAddressCount)
{
	if(puSourceAddress && uSourceCount)
	{
		UINT uAddressBit, uOutputAddressCount = 0;

		if(puOutputAddress)
		{
			for(UINT i = 0; i < uSourceCount; ++ i)
			{
				uAddressBit = puSourceAddress[i];
				if( ALCHEMY_TEST_BIT(uAddress, uAddressBit) )
					puOutputAddress[uOutputAddressCount ++] = uAddressBit;
			}
...
void CEffectCode::Clear()
{
	...
	m_InstructionIndexPool.Clear();
	m_InstructionIndicesPool.Clear();

	m_RenderTargetInstructionPool.Clear();
	m_RenderTargetInstructionSetPool.Clear();
	...
	if(pVertexFunction->uResultVariable && pFragmentFunction->uParametersVariable)
	{
		...
		if(pVertexVariable && pFragmentVariable)
		{
			if(pVertexVariable->Type == STRUCT_VARIABLE && pVertexVariable->uDeclaration && pFragmentVariable->Type == STRUCT_VARIABLE && pFragmentVariable->uDeclaration)
			{
				...
				{
					UINT uInstructionIndices = 0;
					CVector<UINT> Registers;
					UINT uUserVariable = 0, uConstantInstruction = 0, uRenderTargetInstructionHandle = 0, uRegisterIndex, uRegister, uRegisterCount;
					...
					LPRENDERTARGETINSTRUCTION pRenderTargetInstruction = ALCHEMY_NULL;

					RENDERTARGETINSTRUCTION RenderTargetInstruction;
					LPUSERVARIABLE pUserVariable;
					bool bIsUpdateMap;

					uInstructionIndices = AppendCodeInstruction(*puFragmentInstructionSet, &uUserVariable, uResultVariable, true, &uConstantInstruction);
					if(uInstructionIndices && uUserVariable)
					{
						/*wprintf(L"-------------------------Result---------------------------\n");
						wprintf(L"---Fragment Shader---\n");
						wprintf( static_cast<const CHAR*>(Code) );*/
						RenderTargetInstruction.uFragmentInstructionIndices     = m_InstructionIndicesPool.Add(uInstructionIndices) + 1;
						RenderTargetInstruction.uFragmentUserVariables          = uUserVariable ? m_UserVariablesPool.Add(uUserVariable) + 1 : 0;
						RenderTargetInstruction.uVertexInstructionIndices       = 0;
						RenderTargetInstruction.uVertexUserVariables            = 0;
						RenderTargetInstruction.uVertexConstantInstructionSet   = 0;
						RenderTargetInstruction.uFragmentConstantInstructionSet = uConstantInstruction ? (m_InstructionSetPool.Add(uConstantInstruction) + 1) : 0;
						RenderTargetInstruction.uIndex                          = 0;
						RenderTargetInstruction.uNext                           = 0;
								...
								if(pFragmentVariable)
								{
									...
									while(pVertexVariable)
									{
										if(pVertexParameter)
										{
											...
											{
												uInstructionIndices = AppendCodeInstruction(*puVertexInstructionSet, &uUserVariable, uVertexVariable, bIsUpdateMap, &uConstantInstruction);
									...
									if(pVertexVariable)
									{
										...
										while(pVertexVariable && pFragmentVariable)
										{
											...
											uInstructionIndices = AppendCodeInstruction(*puVertexInstructionSet, &uUserVariable, uVertexVariable, false, &uConstantInstruction);
										}
									}
								}
							}
						...
						RenderTargetInstruction.uVertexInstructionIndices     = m_InstructionIndicesPool.Add(uInstructionIndices) + 1;
						RenderTargetInstruction.uVertexUserVariables          = uUserVariable ? (m_UserVariablesPool.Add(uUserVariable) + 1) : 0;

						RenderTargetInstruction.uVertexConstantInstructionSet = uConstantInstruction ? (m_InstructionSetPool.Add(uConstantInstruction) + 1) : 0;

						uRenderTargetInstructionHandle                        = m_RenderTargetInstructionPool.Add(RenderTargetInstruction);
						pRenderTargetInstruction                              = &m_RenderTargetInstructionPool[uRenderTargetInstructionHandle ++];
					}
					...
					if(uResultVariable != pFragmentFunction->uResultVariable)
					{
						...
						while(uResultVariable)
						{
							...
							if(uInstructionIndices && uUserVariable)
							{
								...
								RenderTargetInstruction.uVertexConstantInstructionSet   = 0;
								RenderTargetInstruction.uFragmentConstantInstructionSet = uConstantInstruction ? (m_InstructionSetPool.Add(uConstantInstruction) + 1) : 0;
								RenderTargetInstruction.uIndex                          = uAddress - 1;
								RenderTargetInstruction.uNext                           = 0;
								...
								if(pRenderTargetInstruction)
									pRenderTargetInstruction->uNext                   = m_RenderTargetInstructionPool.Add(RenderTargetInstruction);

								pRenderTargetInstruction                              = &m_RenderTargetInstructionPool[pRenderTargetInstruction->uNext ++];

								if(!uRenderTargetInstructionHandle)
									uRenderTargetInstructionHandle = pRenderTargetInstruction->uNext;
							}
					...
					Pass.uRenderTargetInstructionSet = uRenderTargetInstructionHandle ? m_RenderTargetInstructionSetPool.Add(uRenderTargetInstructionHandle) + 1 : 0;
				}
				else
				{
			...
			{
				UINT uUserVariable = 0, uConstantInstruction = 0, uVertexInstructionIndices, uVertexUserVariables, uVertexConstantInstructionSet;
				RENDERTARGETINSTRUCTION RenderTargetInstruction;
				UINT uInstructionIndices = 0;
				
				uInstructionIndices = AppendCodeInstruction(*puVertexInstructionSet, &uUserVariable, pVertexFunction->uResultVariable, true, &uConstantInstruction);
				uVertexInstructionIndices     = m_InstructionIndicesPool.Add(uInstructionIndices) + 1;
				...
				LPRENDERTARGETINSTRUCTION pRenderTargetInstruction = ALCHEMY_NULL;
				UINT uRenderTargetInstructionHandle = 0;
				uInstructionIndices = AppendCodeInstruction(*puFragmentInstructionSet, &uUserVariable, uResultVariable, true, &uConstantInstruction);
				if(uInstructionIndices && uUserVariable)
				{
					RenderTargetInstruction.uVertexInstructionIndices       = uVertexInstructionIndices;
					RenderTargetInstruction.uVertexUserVariables            = uVertexUserVariables;
					RenderTargetInstruction.uFragmentInstructionIndices     = m_InstructionIndicesPool.Add(uInstructionIndices) + 1;
					RenderTargetInstruction.uFragmentUserVariables          = uUserVariable ? (m_UserVariablesPool.Add(uUserVariable) + 1) : 0;
					RenderTargetInstruction.uVertexConstantInstructionSet   = uVertexConstantInstructionSet;
					RenderTargetInstruction.uFragmentConstantInstructionSet = uConstantInstruction ? (m_InstructionSetPool.Add(uConstantInstruction) + 1) : 0;
					RenderTargetInstruction.uIndex                          = 0;
					RenderTargetInstruction.uNext                           = 0;
...
typedef struct  
{
	UINT uConstantAddress;
	UINT uDestinationAddress;
}CONSTANTADDRESS, * LPCONSTANTADDRESS;

UINT CEffectCode::AppendCodeInstruction(UINT uInstruction, PUINT puUserVaraible, UINT uDestination, bool bIsUpdateMap, PUINT puConstantInstructionSet)
{
	static UINT s_uInstructionIndexHead = 0;
	s_uInstructionIndexHead = bIsUpdateMap ? 0 : s_uInstructionIndexHead;

	if(!uInstruction)
		return s_uInstructionIndexHead;
	...
	static CMap<UINT, CONSTANTADDRESS> s_ConstantAddressMap;

	static CONSTANTADDRESS s_ConstantAddress;
	static GLOBALVARIABLE s_GlobalVariable;
	static INSTRUCTION s_Instruction;
	static INSTRUCTIONINDEX s_InstructionIndex;

	static UINT s_uInstructionIndexHandle = 0;
	...
	if(bIsUpdateMap)
	{
		...
		s_uInstructionIndexHandle = 0;
	}

	if(uDestination || bIsUpdateMap)
	{
		if(uDestination)
			AppendCodeInstruction(pInstruction->uPrevious, puUserVaraible, uDestination, false, puConstantInstructionSet);

		s_bIsUpdateInstructionMap = true;
		AppendCodeInstruction(uInstruction, ALCHEMY_NULL, 0, false, puConstantInstructionSet);
	...
	LPINSTRUCTIONINDEX pInstructionIndex;
	bool bIsConstantInstruction = pInstructionState ? pInstructionState->bIsConstantInstruction : !bIsUpdateMap;
	UINT uConstantInstructionHandle, 
		uConstant, 
		uSourceAddressShift, 
		uDestinationAddressShift, 
		uRegister, 
		uAddress, 
		uAddressCount, 
		uDestinationIndexConstant = 0, 
		uDestinationIndexAddress = pCurrentInstruction->uDestinationIndexAddress, 
		uSourceConstant1 = 0, 
		uSourceAddress1 = 0, 
		uSourceAddressInfo1 = 0, 
		uSourceIndexConstant1 = 0, 
		uSourceIndexAddress1 = pCurrentInstruction->uSourceIndexAddress1, 
		uSourceConstant2 = 0, 
		uSourceAddress2 = 0, 
		uSourceAddressInfo2 = 0, 
		uSourceIndexConstant2 = 0,
		uSourceIndexAddress2 = pCurrentInstruction->uSourceIndexAddress2;
	LPCONSTANTADDRESS pConstantAddress;
		...
		while(pInstruction)
		{
			if(pInstruction->uDestination)
			{
				...
				if(pVariable)
				{
					...
					{
						AppendCodeInstruction(uPreviousInstruction, puUserVaraible, 0, false, puConstantInstructionSet);
									...
									if(pConstantInstruction)
									{
										if(!bIsConstantInstruction)
										{
											if(pConstantInstruction->uDestination == pInstruction->uDestination)
											{
												...
												uAddressCount = ALCHEMY_BIT_COUNT(pInstruction->uDestinationAddress);
												...
												pConstantInstruction->uDestinationAddress      = (ALCHEMY_FLAG(uAddressCount) - 1) << m_Compiler.GetAddressShift(uConstant, *this);
												...
												s_ConstantAddress.uConstantAddress = pConstantInstruction->uDestinationAddress;
												s_ConstantAddress.uDestinationAddress = pInstruction->uDestinationAddress;

												s_ConstantAddressMap.Insert(uConstant, s_ConstantAddress);

												uDestinationIndexConstant = uConstant;

												uDestinationIndexAddress = AndMatchingAddress( pInstruction->uDestinationAddress, pConstantInstruction->uDestinationAddress, ALCHEMY_FLAG(pCurrentInstruction->uDestinationIndexAddress) );
												if(uDestinationIndexAddress)
													uDestinationIndexAddress = ALCHEMY_MININUM_BIT(uDestinationIndexAddress) - 1;
									...
									if(pConstantInstruction)
									{
										if(!bIsConstantInstruction)
										{
											if(pConstantInstruction->uDestination == pInstruction->uDestination)
											{
												...
												uSourceIndexAddress1 = AndMatchingAddress( pInstruction->uDestinationAddress, pConstantInstruction->uDestinationAddress, ALCHEMY_FLAG(pCurrentInstruction->uSourceIndexAddress1) );
												if(uSourceIndexAddress1)
													uSourceIndexAddress1 = ALCHEMY_MININUM_BIT(uSourceIndexAddress1) - 1;
										...
										if(!bIsConstantInstruction)
										{
											if(pConstantInstruction->uDestination == pInstruction->uDestination)
											{
												...
												if( uAddress == pCurrentInstruction->uSourceAddress1 && ALCHEMY_TEST_FLAG(pInstruction->uDestinationAddress, uAddress) )
												{
													...
													uSourceAddress1     = pInstruction->uDestinationAddress == pConstantInstruction->uDestinationAddress ? uAddress : AndMatchingAddress(pInstruction->uDestinationAddress, pConstantInstruction->uDestinationAddress, uAddress);
													uSourceAddressInfo1 = 0;
												...
												{
													s_Instruction.Opcode                   = ICompiler::MOV;
													s_Instruction.uDestination             = pInstruction->uDestination;
													s_Instruction.uDestinationAddress      = pInstruction->uDestinationAddress;
													s_Instruction.uDestinationIndex        = pInstruction->uDestinationIndex;
													s_Instruction.uDestinationIndexAddress = pInstruction->uDestinationIndexAddress;
													s_Instruction.uDestinationAddressInfo  = 0;
													s_Instruction.uSource1                 = pConstantInstruction->uDestination;
													s_Instruction.uSourceAddress1          = pConstantInstruction->uDestinationAddress;
													s_Instruction.uSourceIndex1            = 0;
													s_Instruction.uSourceIndexAddress1     = 0;
													s_Instruction.uSourceAddressInfo1      = 0;
													s_Instruction.uSource2                 = 0;
													s_Instruction.uSourceAddress2          = 0;
													s_Instruction.uSourceIndex2            = 0;
													s_Instruction.uSourceIndexAddress2     = 0;
													s_Instruction.uSourceAddressInfo2      = 0;
													s_Instruction.uPrevious                = 0;

													s_InstructionIndex.uCurrentInstruction = m_InstructionPool.Add(s_Instruction) + 1;
													s_InstructionIndex.uNextIndex          = 0;

													pInstructionIndex  = s_uInstructionIndexHandle ? m_InstructionIndexPool.Get(s_uInstructionIndexHandle - 1) : ALCHEMY_NULL;

													s_uInstructionIndexHandle = m_InstructionIndexPool.Add(s_InstructionIndex) + 1;

													if(pInstructionIndex)
														pInstructionIndex->uNextIndex = s_uInstructionIndexHandle;
													else if(!s_uInstructionIndexHead)
														s_uInstructionIndexHead = s_uInstructionIndexHandle;
												}
											...
											{
												pConstantAddress = s_ConstantAddressMap.Get(pConstantInstruction->uDestination);
												if(pConstantAddress && pConstantAddress->uConstantAddress)
												{
													if( uAddress == pCurrentInstruction->uSourceAddress1 && ALCHEMY_TEST_FLAG(pInstruction->uDestinationAddress, uAddress) )
													{
														uSourceConstant1 = pConstantInstruction->uDestination;

														if(pConstantAddress->uDestinationAddress == pConstantInstruction->uDestinationAddress)
														{
														...
														else
														{
															...
															if(pBufferPointer)
															{
																uSourceAddressShift = m_Compiler.GetAddressShift(pCurrentInstruction->uSource1, *this);
																uDestinationAddressShift = m_Compiler.GetAddressShift(pConstantInstruction->uDestination, *this);
																uSourceAddress1 = AndMatchingAddress(
																	pConstantAddress->uDestinationAddress >> uSourceAddressShift, 
																	pConstantInstruction->uDestinationAddress >> uDestinationAddressShift, 
																	uAddress >> uSourceAddressShift, 
																	reinterpret_cast<const UINT*>( (*pBufferPointer)->GetBufferPointer() ), 
																	(*pBufferPointer)->GetBufferSize() / sizeof(UINT), 
																	ALCHEMY_NULL, 
																	&uAddressCount
																	) << uDestinationAddressShift;

																s_BufferPointer = &CreateBuffer(uAddressCount * sizeof(UINT), true);
																...
																AndMatchingAddress(
																	...
																	reinterpret_cast<const UINT*>( (*pBufferPointer)->GetBufferPointer() ), 
																	(*pBufferPointer)->GetBufferSize() / sizeof(UINT), 
																	reinterpret_cast<PUINT>( s_BufferPointer->GetBufferPointer() ) );

																uSourceAddressInfo1 = m_BufferPool.Add(s_BufferPointer) + 1;
															}
															else
															{
																uSourceAddress1 = AndMatchingAddress(
																	pConstantAddress->uDestinationAddress, 
																	pConstantInstruction->uDestinationAddress, 
																	uAddress);
													...
													{
														s_Instruction.uDestinationAddress = uAddress & pInstruction->uDestinationAddress;
														s_Instruction.uSourceAddress1 = AndMatchingAddress(pConstantAddress->uDestinationAddress, pConstantInstruction->uDestinationAddress, s_Instruction.uDestinationAddress) & pConstantAddress->uConstantAddress;
														if(s_Instruction.uSourceAddress1)
														{
															ALCHEMY_CLEAR_FLAG(pConstantAddress->uConstantAddress, s_Instruction.uSourceAddress1);

															s_Instruction.uDestinationAddress = AndMatchingAddress(pConstantInstruction->uDestinationAddress, pConstantAddress->uDestinationAddress, s_Instruction.uSourceAddress1);

															if(uSourceIndexConstant1)
															{
																s_Instruction.uDestinationIndex        = uSourceIndexConstant1;
																s_Instruction.uDestinationIndexAddress = uSourceIndexAddress1;
															}
															else
															{
																s_Instruction.uDestinationIndex        = pCurrentInstruction->uSourceIndex1;
																s_Instruction.uDestinationIndexAddress = pCurrentInstruction->uSourceIndexAddress1;
															}

															s_Instruction.Opcode                   = ICompiler::MOV;
															s_Instruction.uDestination             = pCurrentInstruction->uSource1;
															s_Instruction.uDestinationAddressInfo  = 0;
															s_Instruction.uSource1                 = pConstantInstruction->uDestination;
															s_Instruction.uSourceIndex1            = pConstantInstruction->uDestinationIndex;
															s_Instruction.uSourceIndexAddress1     = pConstantInstruction->uDestinationIndexAddress;
															s_Instruction.uSourceAddressInfo1      = 0;
															s_Instruction.uSource2                 = 0;
															s_Instruction.uSourceAddress2          = 0;
															s_Instruction.uSourceIndex2            = 0;
									...
									if(pConstantInstruction)
									{
										if(!bIsConstantInstruction)
										{
											if(pConstantInstruction->uDestination == pInstruction->uDestination)
											{
												...
												uSourceIndexAddress2 = AndMatchingAddress( pInstruction->uDestinationAddress, pConstantInstruction->uDestinationAddress, ALCHEMY_FLAG(pCurrentInstruction->uSourceIndexAddress2) );
												if(uSourceIndexAddress2)
													uSourceIndexAddress2 = ALCHEMY_MININUM_BIT(uSourceIndexAddress2) - 1;
										...
										if(!bIsConstantInstruction)
										{
											if(pConstantInstruction->uDestination == pInstruction->uDestination)
											{
												...
												if( uAddress == pCurrentInstruction->uSourceAddress2 && ALCHEMY_TEST_FLAG(pInstruction->uDestinationAddress, uAddress) )
												{
													...
													uSourceAddress2     = pInstruction->uDestinationAddress == pConstantInstruction->uDestinationAddress ? uAddress : AndMatchingAddress(pInstruction->uDestinationAddress, pConstantInstruction->uDestinationAddress, uAddress);
													uSourceAddressInfo2 = 0;
												...
												if(pConstantAddress && pConstantAddress->uConstantAddress)
												{
													if( uAddress == pCurrentInstruction->uSourceAddress2 && ALCHEMY_TEST_FLAG(pInstruction->uDestinationAddress, uAddress) )
													{
														...
														else
														{
															...
															if(pBufferPointer)
															{
																uSourceAddressShift = m_Compiler.GetAddressShift(pCurrentInstruction->uSource2, *this);
																uDestinationAddressShift = m_Compiler.GetAddressShift(pConstantInstruction->uDestination, *this);
																uSourceAddress2 = AndMatchingAddress(
																	pConstantAddress->uDestinationAddress >> uSourceAddressShift, 
																	pConstantInstruction->uDestinationAddress >> uDestinationAddressShift, 
																	uAddress >> uSourceAddressShift, 
																	reinterpret_cast<const UINT*>( (*pBufferPointer)->GetBufferPointer() ), 
																...
																uSourceAddressInfo2 = m_BufferPool.Add(s_BufferPointer) + 1;
															}
															else
															{
																uSourceAddress2 = AndMatchingAddress(
													...
													else
													{
														...
														if(s_Instruction.uSourceAddress1)
														{
															...
															{
																s_Instruction.uDestinationIndex        = uSourceIndexConstant2;
																s_Instruction.uDestinationIndexAddress = uSourceIndexAddress2;
															}
															else
															{
																s_Instruction.uDestinationIndex        = pCurrentInstruction->uSourceIndex2;
																s_Instruction.uDestinationIndexAddress = pCurrentInstruction->uSourceIndexAddress2;
															}

															s_Instruction.Opcode                   = ICompiler::MOV;
															s_Instruction.uDestination             = pCurrentInstruction->uSource2;
															s_Instruction.uDestinationAddressInfo  = 0;
															s_Instruction.uSource1                 = pConstantInstruction->uDestination;
															s_Instruction.uSourceIndex1            = pConstantInstruction->uDestinationIndex;
															s_Instruction.uSourceIndexAddress1     = pConstantInstruction->uDestinationIndexAddress;
	...
	if(bIsConstantInstruction)
	{
		...
		if(RegisterType == ICompiler::RESULT || RegisterType == ICompiler::OUTPUT)
		{
			...
			else
			{
				...
				uAddressCount = ALCHEMY_BIT_COUNT(pCurrentInstruction->uDestinationAddress);
				...
				s_Instruction.Opcode                   = ICompiler::MOV;
				s_Instruction.uDestination             = pCurrentInstruction->uDestination;
				s_Instruction.uDestinationAddress      = pCurrentInstruction->uDestinationAddress;
				s_Instruction.uDestinationIndex        = pCurrentInstruction->uDestinationIndex;
				s_Instruction.uDestinationIndexAddress = pCurrentInstruction->uDestinationIndexAddress;
				s_Instruction.uDestinationAddressInfo  = 0;
				s_Instruction.uSource1                 = uConstant;
				s_Instruction.uSourceAddress1          = (ALCHEMY_FLAG(uAddressCount) - 1) << m_Compiler.GetAddressShift(uConstant, *this);
				s_Instruction.uSourceIndex1            = 0;
				s_Instruction.uSourceIndexAddress1     = 0;
				s_Instruction.uSourceAddressInfo1      = 0;
				s_Instruction.uSource2                 = 0;
	...
	if(bIsConstantInstruction)
	{
		...
		if(uConstant)
		{
			...
			ConstantInstruction.uDestinationAddress     = s_Instruction.uSourceAddress1;
		...
		else if(pCurrentInstruction->Opcode == ICompiler::MOV)
		{
			if(uSourceConstant1)
			{
				...
				ConstantInstruction.uDestinationAddress     = uSourceAddress1;
				ConstantInstruction.uDestinationAddressInfo = 0;
			}
			else
			{
				...
				ConstantInstruction.uDestinationAddressInfo = 0;
				ConstantInstruction.uSourceAddressInfo1     = 0;
			}

			if(uSourceIndexConstant1)
			{
				ConstantInstruction.uDestinationIndex        = uSourceIndexConstant1;
				ConstantInstruction.uDestinationIndexAddress = uSourceIndexAddress1;
			}
			else
			{
			...
			s_ConstantAddress.uConstantAddress = ConstantInstruction.uDestinationAddress;
			s_ConstantAddress.uDestinationAddress = pCurrentInstruction->uDestinationAddress;

			s_ConstantAddressMap.Insert(ConstantInstruction.uDestination, s_ConstantAddress);
	...
	else
	{
		if(pCurrentInstruction->Opcode == ICompiler::TEX && !pCurrentInstruction->uSource2)
		{
			...
			s_InstructionIndex.uCurrentInstruction = m_InstructionPool.Add(s_Instruction);

			pCurrentInstruction = m_InstructionPool.Get(s_InstructionIndex.uCurrentInstruction ++);
		}
		else
			s_InstructionIndex.uCurrentInstruction = uInstruction;

		if(uDestinationIndexConstant || uSourceConstant1 || uSourceIndexConstant1 || uSourceConstant2 || uSourceIndexConstant2)
		{
			if(s_InstructionIndex.uCurrentInstruction == uInstruction)
			{
				s_Instruction = *pCurrentInstruction;
			...
			{
				pCurrentInstruction->uDestinationIndex         = uDestinationIndexConstant;
				pCurrentInstruction->uDestinationIndexAddress  = uDestinationIndexAddress;
			}

			if(uSourceConstant1)
			{
				pCurrentInstruction->uSource1            = uSourceConstant1;
				pCurrentInstruction->uSourceAddress1     = uSourceAddress1;
				pCurrentInstruction->uSourceAddressInfo1 = uSourceAddressInfo1;
			}

			if(uSourceIndexConstant1)
			{
				pCurrentInstruction->uSourceIndex1        = uSourceIndexConstant1;
				pCurrentInstruction->uSourceIndexAddress1 = uSourceIndexAddress1;
			}

			if(uSourceConstant2)
			{
				pCurrentInstruction->uSource2            = uSourceConstant2;
				pCurrentInstruction->uSourceAddress2     = uSourceAddress2;
				pCurrentInstruction->uSourceAddressInfo2 = uSourceAddressInfo2;
			}

			if(uSourceIndexConstant2)
			{
				pCurrentInstruction->uSourceIndex2        = uSourceIndexConstant2;
				pCurrentInstruction->uSourceIndexAddress2 = uSourceIndexAddress2;
			}
		}
...
UINT CEffectCode::AndMatchingAddress(
									 UINT uSource, 
									 UINT uDestination, 
									 UINT uTarget, 
									 const UINT* puSourceAddress, 
									 UINT uSourceAddressCount, 
									 PUINT puOutputAddress, 
									 PUINT puOutputAddressCount)
{
	UINT uResult = 0, uSourceBit, uDestinationBit;

	if( puSourceAddress && uSourceAddressCount && (puOutputAddress || puOutputAddressCount) )
	{
		PUINT puAddress;
		ALCHEMY_DEBUG_NEW(puAddress, UINT[uSourceAddressCount]);

		UINT i;
		while(uSource && uDestination)
		{
			uSourceBit      = ALCHEMY_MININUM_BIT(uSource)      - 1;
			uDestinationBit = ALCHEMY_MININUM_BIT(uDestination) - 1;

			if( ALCHEMY_TEST_BIT(uTarget, uSourceBit) )
			{
				ALCHEMY_SET_BIT(uResult, uDestinationBit);

				for(i = 0; i < uSourceAddressCount; ++ i)
				{
					if(puSourceAddress[i] == uSourceBit)
						puAddress[i] = uDestinationBit;
				}
			}

			ALCHEMY_CLEAR_BIT(uSource     , uSourceBit     );
			ALCHEMY_CLEAR_BIT(uDestination, uDestinationBit);
		}

		UnserializeAddress(uResult, puAddress, uSourceAddressCount, puOutputAddress, puOutputAddressCount);

		ALCHEMY_DEBUG_DELETE_ARRAY(puAddress);
	}
	else
	{
		while(uSource && uDestination)
...
CBuffer& CEffectCode::CreatePassInfos(PUINT puRenderTargetIndexCount)
{
	...
	const UINT* puRenderTargetInstructionSet;
	const PASS* pPass;
	LPRENDERTARGETINSTRUCTION pRenderTargetInstruction;
	for(i = 0; i < uPassCount; ++ i)
	{
		pPass = m_Passes.Get(i);
		uRenderTargetIndexCount = 0;
		if(pPass && pPass->uRenderTargetInstructionSet && ( puRenderTargetInstructionSet = m_RenderTargetInstructionSetPool.Get(pPass->uRenderTargetInstructionSet - 1) ) && *puRenderTargetInstructionSet)
		{
			pRenderTargetInstruction = m_RenderTargetInstructionPool.Get(*puRenderTargetInstructionSet - 1);
			while(pRenderTargetInstruction)
			{
				++ uRenderTargetIndexCount;

				pRenderTargetInstruction = pRenderTargetInstruction->uNext ? m_RenderTargetInstructionPool.Get(pRenderTargetInstruction->uNext - 1) : ALCHEMY_NULL;
			}
		}
	...
	for(i = 0; i < uPassCount; ++ i)
	{
		...
		if(pPass && pPass->uRenderTargetInstructionSet && ( puRenderTargetInstructionSet = m_RenderTargetInstructionSetPool.Get(pPass->uRenderTargetInstructionSet - 1) ) && *puRenderTargetInstructionSet)
		{
			...
			while(pRenderTargetInstruction)
			{
				...
				puUserVariable = pRenderTargetInstruction->uVertexUserVariables ? m_UserVariablesPool.Get(pRenderTargetInstruction->uVertexUserVariables - 1) : ALCHEMY_NULL;
				...
				while(pUserVariable)
				{
					...
					if(!pUserVariable && Type == IEffect::VERTEX_SHADER)
					{
						...
						puUserVariable = pRenderTargetInstruction->uFragmentUserVariables/*pFragmentShaderFunction->uUserVariables*/ ? m_UserVariablesPool.Get(pRenderTargetInstruction->uFragmentUserVariables/*pFragmentShaderFunction->uUserVariables*/ - 1) : ALCHEMY_NULL;
						pUserVariable  = (puUserVariable && *puUserVariable) ? m_UserVariablePool.Get(*puUserVariable - 1) : ALCHEMY_NULL;
					}
				}
				...
				while(pUserVariable)
				{
					...
					if(!pUserVariable && Type == IEffect::VERTEX_SHADER)
					{
						...
						puUserVariable = pRenderTargetInstruction->uFragmentUserVariables ? m_UserVariablesPool.Get(pRenderTargetInstruction->uFragmentUserVariables - 1) : ALCHEMY_NULL;
						pUserVariable  = (puUserVariable && *puUserVariable) ? m_UserVariablePool.Get(*puUserVariable - 1) : ALCHEMY_NULL;
					}
				}
...
CBuffer& CEffectCode::CreateParameterList(CGlobalInputHandler** ppGlobalInputHandler, PUINT puRenderTargetIndexCount)
{
	...
	for(i = 0; i < uPassCount; ++ i)
	{
		...
		if(pPass && pPass->uRenderTargetInstructionSet && ( puRenderTargetInstructionSet = m_RenderTargetInstructionSetPool.Get(pPass->uRenderTargetInstructionSet - 1) ) && *puRenderTargetInstructionSet)
		{
			...
			while(pRenderTargetInstruction)
			{
				if( ppGlobalInputHandler && (pRenderTargetInstruction->uVertexConstantInstructionSet || pRenderTargetInstruction->uFragmentConstantInstructionSet) )
				{
					puConstantInstruction = m_InstructionSetPool.Get(pRenderTargetInstruction->uVertexConstantInstructionSet - 1);
					if(puConstantInstruction)
						Type = IEffect::VERTEX_SHADER;
					else
					{
						puConstantInstruction = m_InstructionSetPool.Get(pRenderTargetInstruction->uFragmentConstantInstructionSet - 1);
				...
				puUserVariable = pRenderTargetInstruction->uVertexUserVariables/*pVertexShaderFunction->uUserVariables*/ ? m_UserVariablesPool.Get(pRenderTargetInstruction->uVertexUserVariables/*pVertexShaderFunction->uUserVariables*/ - 1) : ALCHEMY_NULL;
				pUserVariable  = (puUserVariable && *puUserVariable) ? m_UserVariablePool.Get(*puUserVariable - 1) : ALCHEMY_NULL;
				Type = IEffect::VERTEX_SHADER;
				uMinRegister                  = ~0;
				uVertexSamplerCount           = 0;
