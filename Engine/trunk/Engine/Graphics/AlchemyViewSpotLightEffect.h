...
namespace alchemy
{
		...
		public EFFECTINFO
	{
		...
		typedef enum
		{
			CONSTANT_LIGHT_FALLOFF = 0,
			CONSTANT_LIGHT_COS_THETA,
			CONSTANT_LIGHT_INVERSE_COS_PHI_SUBSTRUCT_COS_THETA,

			CONSTANT_LIGHT_VIEW_POSITION,
			CONSTANT_LIGHT_VIEW_DIRECTION,
		...
		typedef enum
		{
			...
			CONSTANT_VARIABLE_LIGHT_VIEW_DIRECTION,

			TOTAL_CONSTANT_VARIABLES
		}CONSTANTVARIABLEINDEX;
	public:
		CViewSpotLightEffect(UINT uLightIndex);
		~CViewSpotLightEffect(void);
