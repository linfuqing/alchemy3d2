...
namespace alchemy
{
		...
		public EFFECTINFO
	{
		...
		typedef enum
		{
			CONSTANTS_CAMERA_LOCAL_POSITION = 0,
		...
		typedef enum
		{
			CONSTANT_VARIABLE_CAMERA_LOCAL_DIRECTION = 0,

			CONSTANT_VARIABLE_CAMERA_LOCAL_DIRECTION_NEGATE,

			TOTAL_CONSTANT_VARIABLES
		}CONSTANTVARIABLEINDEX;
	public:
		CCameraDirectionEffect(IEffect::EFFECTTYPE EffectType = IEffect::VERTEX_SHADER);
		~CCameraDirectionEffect(void);
