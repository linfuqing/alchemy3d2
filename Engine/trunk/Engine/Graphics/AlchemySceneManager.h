...
namespace alchemy
{
	...
	public:
		...
		CSceneNode* CreateScene(
			CMeshManager::HANDLE hSceneDataHandle, 
			CSkinMeshController::SKINTYPE SkinType = CSkinMeshController::SKIN_SOFTWARE, 
			CVector<CSceneObject*>* pSceneObjects = ALCHEMY_NULL, 
			CVector<CBone*>* pRootBone = ALCHEMY_NULL, 
			CVector<RESOURCEEINFO>* pMaterialInfo = ALCHEMY_NULL, 
			CVector<RESOURCEEINFO>* pTextureInfo = ALCHEMY_NULL, 
			CVector<RESOURCEEINFO>* pMeshInfo = ALCHEMY_NULL, 
			ISceneNodeFactory* pSceneNodeFactory = ALCHEMY_NULL);
