...
void CWin32Loader::Load(const alchemy::CHAR* pcPath)
{
	...
	*/
	...
	if( (pInFile = fopen(pcCurrentPath, "rb") ) == ALCHEMY_NULL) 
	{
		fprintf(stderr, "can't open %s\n", pcCurrentPath);

#ifdef _UNICODE
		ALCHEMY_DEBUG_DELETE_ARRAY(pcCurrentPath);
#endif
	...
	else
	{
		...
		Data.uAlpha = ftell(pInFile);
		Data.uBeta  = 0;

		ALCHEMY_DEBUG_NEW(Data.pData, UINT8[Data.uAlpha]);

		fseek(pInFile, 0, SEEK_SET);
		fread(Data.pData, 1, Data.uAlpha, pInFile);
	}
	...
	{
		CLoadEvent Event(Data, pcPath, uType);
