...
CFlasccIndexBuffer3D::CFlasccIndexBuffer3D(void) :
...
void* CFlasccIndexBuffer3D::Lock(UINT uFlag)
{
	if( ALCHEMY_TEST_BIT(uFlag, INDEXBUFFER::READ_ONLY) )
		return ALCHEMY_TEST_BIT(GetUsage(), INDEXBUFFER::WRITE_ONLY) ? ALCHEMY_NULL : m_pData;
	...
	if(!m_pData)
		ALCHEMY_DEBUG_NEW(m_pData, UINT8[m_uDataSize = INDEXBUFFER::GetByteSize()]);

	return m_pData;
}

void* CFlasccIndexBuffer3D::Lock(UINT uStart, UINT uEnd, UINT uFlag)
{
	if(uStart < uEnd)
	{
		UINT uUsage = GetUsage();
		if( ALCHEMY_TEST_BIT(uFlag, INDEXBUFFER::READ_ONLY) )
			return ALCHEMY_TEST_BIT(uUsage, INDEXBUFFER::WRITE_ONLY) ? ALCHEMY_NULL : reinterpret_cast<PUINT8>( m_pData + uStart * GetStride() );
...
bool CFlasccIndexBuffer3D::Unlock()
{
	...
	inline_as3(
		...
		"if(renderSystem) renderSystem.uploadIndexBuffer3D(%1, ram, %2, %3, %4);\n"
		: : "r"( static_cast<CFlasccContext3D&>( GetGraphicsResourcePool() ).GetRenderSystemIndex() ),
		"r"( static_cast<unsigned int>( GetResourceHandle() ) ), 
		"r"( ( !ALCHEMY_TEST_BIT(uUsage, INDEXBUFFER::WRITE_ONLY) || ALCHEMY_TEST_BIT(uUsage, INDEXBUFFER::DYNAMIC) ) ? reinterpret_cast<PUINT8>(m_pData) + ( m_uLockStart * GetStride() ) : m_pData ), 
		"r"(m_uLockStart), 
		"r"(m_uLockCount)
		);
#endif
...
void CFlasccIndexBuffer3D::Destroy()
{
	ALCHEMY_DEBUG_DELETE_ARRAY(m_pData);
}

bool CFlasccIndexBuffer3D::_Create()
{
#ifdef AS3_H
	inline_as3(
		"import alchemy.core.AlchemyGraphicsManager;\n"
		"import alchemy.core.AlchemyRenderSystem;\n"
		"var renderSystem = AlchemyGraphicsManager.instance.getRenderSystem(%0);\n"
		"if(renderSystem) renderSystem.createIndexBuffer3D(%1);\n"
		: : "r"( static_cast<CFlasccContext3D&>( GetGraphicsResourcePool() ).GetRenderSystemIndex() ), "r"( GetMemberCount() )
		);
#endif
