...
bool CMeshManager::DestroySceneNodeData(HANDLE uHandle)
{
	...
	if(pSceneNodeData)
	{
		__DestroySceneNodeData(*pSceneNodeData);
...
bool CMeshManager::DestroyTextureData(HANDLE uHandle)
{
	...
	if(pTextureData)
	{
		ALCHEMY_DEBUG_DELETE_ARRAY(pTextureData->pData);
		ALCHEMY_DEBUG_DELETE_ARRAY(pTextureData->pcFileName);
...
bool CMeshManager::DestroySkinData(HANDLE uHandle)
{
	...
	__DestroySkinData( *m_SkinPool.Get(uHandle - 1) );
...
void CMeshManager::Destroy()
{
	...
	{
		__DestroyTextureData(*TextureIterator);
	...
	{
		__DestroyMaterialData(*MaterialIterator);
	...
	{
		__DestroySkinData(*SkinDataIterator);
	...
	{
		__DestroySceneNodeData(*SceneNodeIterator);
...
void CMeshManager::__DestroyTextureData(CMeshManager::TEXTUREDATA& TextureData)const
{
	ALCHEMY_DEBUG_DELETE_ARRAY(TextureData.pData);
}

void CMeshManager::__DestroyMaterialData(MATERIALDATA& MaterialData)const
{
...
void CMeshManager::__DestroySkinData(CMeshManager::SKINDATA& SkinData) const
{
	UINT i;
...
void CMeshManager::__DestroySceneNodeData(SCENENODEDATA& SceneNodeData)const
{
